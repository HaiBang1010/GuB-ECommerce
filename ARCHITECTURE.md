...
3. **Post-purchase** — **DONE** (reviews · notifications · admin foundation; verified locally, deploy deferred). Order timeline already shipped in Phase 2.
   ...
   **Admin — DONE** (foundation + orders management). A single `/admin` **route group** (`(admin)` vs `(storefront)`, URL-transparent so customer URLs are unchanged) with a client shell whose role comes from **`GET /me`** (reads `iam.User.role` from the DB — **no** Supabase-metadata sync), gated for real by `middleware.ts` (session required) + the backend **`RoleGuard`** (403 for non-admins). **Admin orders** (`GET /admin/orders`): customer info **enriched in-process via `UserService`** (no cross-schema JOIN, §4.3), a **multi-status filter**, a **unified search** (order id / customer name / email), and **server-side pagination** (`page`/`pageSize` + `total`). The **admin review-reply input** ships inline on the product page (`POST /admin/reviews/:id/reply`) — there is no admin list-all-reviews endpoint yet.
   **Remaining / deferred:** **deploy** (Render/Vercel/Neon) not done — the DB is still local Postgres (Docker); **UptimeRobot keep-alive + the `release-expired` cron are not wired** (the endpoint is only triggered manually); **full QStash→email e2e** needs a public consumer URL + real keys (deferred with deploy — the in-process degrade proves the logic locally); an **admin reviews list-all page** and an **admin user-detail page** (`/admin/users/[id]` is a placeholder 404) are later slices; **notification realtime** is deferred to **Phase 6**.
