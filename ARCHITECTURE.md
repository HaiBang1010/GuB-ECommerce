...
| Home banners | **New `marketing` module/schema owns `Banner`**; the image is an **external URL** (admin pastes it — **no upload**) | A banner isn't a catalog/order concept, so a dedicated module keeps the schema-per-module boundary clean. Skipping upload (just a URL string + delivery host's optimization) avoids a Cloudinary signing/persist flow for a low-churn surface; a broken/empty URL degrades to a placeholder on the storefront. |
| Home sections | **Static layout, dynamic content via admin** — a fixed section set (category grid · featured-collection carousels · on-sale · new-arrivals); **NO page builder** | The shape is curated in code; admins drive only *what fills it* — flag a collection `featuredOnHome` (+ `homeSortOrder`) and paste a `Category`/`Collection` `imageUrl`. A drag-and-drop page builder is overkill for a portfolio. The home blends **both** axes: auto rules (on-sale / newest) **and** admin-featured collections. Section images are **pasted URLs (no upload** — Cloudinary upload stays a future option, like Home banners above). Every section **self-hides when empty**, so the home never shows a blank row. |
| Realtime chat transport | **Supabase Realtime _Broadcast_** (not `postgres_changes`), **persist-first** — the backend writes to **Neon (source of truth) via REST**, Broadcast is only a push layer | The chat tables live in **Neon**, so `postgres_changes` (Supabase-DB-only) can't stream them. A dropped/duplicate realtime event is harmless — the client refetches/polls the persisted truth. |
| Chat channel authorization | **Real security, not obscurity**: a per-customer **private** channel `chat:user:<userId>` gated by an **RLS SELECT policy** on `realtime.messages` (`topic = 'chat:user:' \|\| auth.uid()`); the **backend is the sole broadcaster** (service-role). Admin side = **polling**, no channel | `auth.uid()` from the verified Supabase JWT means a customer receives only on their own channel — not a guessable id. The policy authorizes from topic + JWT alone (no Neon lookup). Admin realtime would need an admin identity the RLS lacks → deferred to a **token-mint** upgrade; polling has no security cost (RolesGuard REST is the gate). The RLS policy is a **manual Supabase deploy step**, not a Prisma migration. |
...
```
Category ──┐ (self-ref parentId; archive cascades to children; imageUrl → home grid)
           └─< Product ──< ProductVariant   (size+color, per-variant priceCents+stockQty)
                  │                          (Product.salePriceCents = product-level sale, flat)
                  │     └─< ProductImage     (url, color, position; list reads derive primaryImageUrl)
                  └──< ProductCollection >── Collection   (n-n; season = validFrom/To;
                                                           imageUrl + featuredOnHome + homeSortOrder → home showcase)
...
12. **Review/chat spam** — rate-limit writes. **Chat: DONE** (Phase 6 — `@nestjs/throttler`, per-user, 5 msgs/10s on the customer-send + admin-reply endpoints). Reviews still rely on the purchased-only + one-per-product gate (throttler deferred there).
