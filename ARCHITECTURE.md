...
1. **Catalog** — **IN PROGRESS.** Done: **Category · Product · ProductVariant · Collection · ProductImage** slices — admin CRUD + cascade/season-window archive, storefront reads, size×color variant matrix (per-variant `stockQty` + unique `sku`), n-n Collection membership with season window, by-color images via **Cloudinary signed direct upload** (delivery-time URL transforms, no backend image processing), temporary AdminGuard, jest specs throughout. Cross-slice validation goes through in-process service calls (no cross-table queries). Remaining: **full-text search** (tsvector + GIN + `pg_trgm`) — deferred to a new session. (No login required.)
