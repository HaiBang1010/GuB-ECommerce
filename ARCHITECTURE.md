...
2. **Cart + Auth + Checkout** *(hardest)* — **BACKEND VERIFIED end-to-end with Stripe test mode** (idempotency + oversell proven via real requests, not just unit tests).
   ...
   **Verified end-to-end** (real Supabase project + Stripe test mode, real HTTP requests): full flow cart → order → atomic reservation (stock 5→4) → PaymentIntent → confirm (`pm_card_visa`) → webhook → order `PAID`; **idempotency** — a re-sent `payment_intent.succeeded` leaves the order PAID once, timeline not duplicated, stock still 4 (`StripeEvent` ledger works); **oversell** — ordering `quantity > stock` is blocked, stock never goes negative (atomic decrement); **auth** — Supabase JWKS verification, `RoleGuard` (CUSTOMER → 403 / ADMIN → 200), first-login upsert.
   **Not started / not yet run:** the entire frontend (cart/checkout UI, Stripe.js); deploy (Render/Vercel) not done; the `release-expired` cron not yet wired on UptimeRobot (the endpoint was only triggered manually). Deferred to Phase 4: refund / admin-cancel of a PAID order, voucher discount at checkout.
