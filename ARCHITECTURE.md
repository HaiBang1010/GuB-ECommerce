...
3. **Post-purchase** *(in progress)* — order timeline already shipped in Phase 2.
   ...
   **Remaining:** notifications (async QStash → Resend, the one async path); the admin reply **input** UI (display ships now, but the input needs an `/admin` route group + client-side role — its own slice).
